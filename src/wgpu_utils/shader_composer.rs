...
impl From<bool> for ShaderDefValue {
    fn from(b: bool) -> ShaderDefValue { ShaderDefValue::Bool(b) }
}
impl From<i32> for ShaderDefValue {
    fn from(i: i32) -> ShaderDefValue { ShaderDefValue::Int(i) }
}
impl From<u32> for ShaderDefValue {
    fn from(u: u32) -> ShaderDefValue { ShaderDefValue::UInt(u) }
}
...
impl ShaderComposer {
    ...
    pub fn defines(&self) -> &HashMap<String, compose::ShaderDefValue> { &self.defines }

    pub fn add_shader_define(&mut self, name: &str, value: ShaderDefValue) { self.defines.insert(name.to_string(), value.into()); }
