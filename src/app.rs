    ...
    match event {
        Event::WindowEvent { ref event, .. } => match event {
            WindowEvent::Resized(physical_size) => {
                ...
                log::warn!("Resized to {:?}", physical_size);
            },
            ...
            WindowEvent::RedrawRequested => {
                ...
                let full_output = app_state.egui_renderer.end_frame();
...
pub fn render_app(app: &mut impl App, app_state: &mut AppState, egui_output: egui::FullOutput) -> Result<(), wgpu::SurfaceError> {
    ...
    let screen_descriptor = ScreenDescriptor {
        size_in_pixels: [window_dimensions.width, window_dimensions.height],
